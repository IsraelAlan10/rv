...
public class ObjectController : MonoBehaviour
{
    ...
    /// <summary>
    /// Variables para medir el tiempo y  generar interactividad
    /// </summary>
    /// 

    private float tiempoDecrementar = 4.1f;
    ...
    public void Update()
    {
        if (status == 1)
        {
            if (tiempoActual >= 0)
            {
                tiempoActual = tiempoDecrementar - (Time.time - tiempoInicial);
                GameObject.Find("GraphicsAPIText").GetComponent<GraphicsAPITextController>().Escribe(tiempoActual.ToString("F2"));
            }
            else //Teletransporta
            {
                ...
                {
                    case "Portal12":
                        playerVR.transform.position = new Vector3(60, 1.6f, 0);
                        
                        break;
                    case "Portal13":
                        playerVR.transform.position = new Vector3(35, 1.6f, 30);
                        break;
                    case "Portal21":
